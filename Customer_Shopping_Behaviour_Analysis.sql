...
select item_purchased as Products, round(cast(avg(review_rating) AS numeric), 2) as "Average Product Rating"
from customer
...
with item_counts as(
select category, item_purchased as product, 
count(*) as total_products, 
rank() over(partition by category order by count(*) desc) as item_rank
from customer
group by category, product)

select category, product, total_products 
from item_counts 
where item_rank<=3 
